...
<div class="container">
  ...
  <img  class="myimg"  onclick="getDetails(<?= $myId ?>,'<?= $productName ?>',<?=$productPrice?>,event)" src="<?='prouductphotos/'.$product['img']?>" class="card-img-top" alt="...">
  <div class="card-body">
...
<script> 
let img_id;
    function  getDetails(id,name,price,event){  
      // console.log(event.target.onclick);
      event.target.onclick=null;   
      // console.log(event.target.onclick);
      //   console.log(event.target,id,name,price);  
      div_container= document.createElement("div.container");
     img_id=id;
      prod_name=document.createElement("span.productName"); 
      prod_name.innerHTML=name;
      prod_qty=document.createElement("input"); 
       prod_qty.type="number";  
       prod_qty.value=1;
    //   prod_qty.type="number"; 
      total_prod_price=document.createElement("span.total_prod_price");   
      total_prod_price.innerHTML=price;
      div_container.append( prod_name);
      div_container.append( prod_qty);
      div_container.append( total_prod_price);
      document.getElementById("myDIV").append(div_container); 
      prod_qty.addEventListener("change",function(e){ 
        // console.log(e,e.target.nextSibling);
        e.target.nextSibling.innerHTML=price*e.target.value; 
      }) 
    ...
    function setorder(){
        ...
        let prod_arr=[]; 
         total_price=0;
   containr_arr= document.getElementsByTagName("div.container")
   ...
   total_price+=Number(containr_arr[i].children[2].innerHTML);
